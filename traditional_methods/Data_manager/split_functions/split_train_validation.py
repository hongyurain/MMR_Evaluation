...
def split_train_validation_percentage_user_wise_yinan(URM_train, URM_validation, verbose=True):
    ...
    for user_id in range(URM_train.shape[0]):
        ...
        train_items = user_profile_items[indices_for_sampling]
        train_ratings = user_profile_ratings[indices_for_sampling]

        v_start_pos = URM_validation.indptr[user_id]
        v_end_pos = URM_validation.indptr[user_id + 1]
        v_user_profile_items = URM_validation.indices[v_start_pos:v_end_pos]
        v_user_profile_ratings = URM_validation.data[v_start_pos:v_end_pos]
        v_user_profile_length = len(v_user_profile_items)

        v_indices_for_sampling = np.arange(0, v_user_profile_length, dtype=np.int)
        validation_items = v_user_profile_items[v_indices_for_sampling]
        validation_ratings = v_user_profile_ratings[v_indices_for_sampling]
