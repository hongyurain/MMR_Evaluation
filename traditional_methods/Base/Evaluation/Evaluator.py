...
class Evaluator(object):
    ...
    def evaluateRecommender(self, recommender_object):
        ...
        # results_dict, user_list_jing, model_pred_jing = self._run_evaluation_on_selected_users(recommender_object, self.users_to_evaluate)
        ...
        return (results_dict, results_run_string)#, user_list_jing, model_pred_jing
    ...
    def _compute_metrics_on_recommendation_list(self, test_user_batch_array, recommended_items_batch_list, scores_batch, results_dict):
        ...
        # print('Tag Jing')
        # print('len test_user_batch_array: {}'.format(len(test_user_batch_array)))
        # print('test_user_batch_array: {}'.format(test_user_batch_array))
        # print('recommended_items_batch_list: {}'.format(recommended_items_batch_list))
        # print('len recommended_items_batch_list: {}'.format(len(recommended_items_batch_list)))
        # print('scores_batch: {}'.format(scores_batch))
        # print('len scores_batch: {}'.format(len(scores_batch)))
        # input('debug')
        ...
        for batch_user_index in range(len(recommended_items_batch_list)):
            ...
            for cutoff in self.cutoff_list:
                ...
                results_current_cutoff[EvaluatorMetrics.HIT_RATE.value]             += min(1, is_relevant_current_cutoff.sum())
...
class EvaluatorHoldout(Evaluator):
    ...
    def __init__(self, URM_test_list, cutoff_list, dataset_name_jing, dataset_is_valid_jing, min_ratings_per_user=1, exclude_seen=True,
                 diversity_object = None,
                 ignore_items = None,
                 ignore_users = None,
                 verbose=True):
        ...
        self.times_jing = 0
        self.dataset_name_jing = dataset_name_jing
        self.dataset_is_valid_jing = dataset_is_valid_jing
        self.datetime_jing = datetime.now().strftime('%Y-%m-%d-%H:%M:%S.%f')
    ...
    def _run_evaluation_on_selected_users(self, recommender_object, users_to_evaluate, block_size = None):
        ...
        # print('len scores_batch: {}'.format(len(scores_batch)))
        # print('scores_batch: {}'.format(scores_batch))
        # print('len users_to_evaluate: {}'.format(len(users_to_evaluate)))
        # print('users_to_evaluate: {}'.format(users_to_evaluate))
        # print(self.n_items)
        # input('debug')
        user_list_jing = np.array([-1])
        model_pred_jing = np.array([-1])
        ...
        while user_batch_start < len(users_to_evaluate):
            ...
            if user_batch_start == 0:
                user_list_jing = test_user_batch_array
                model_pred_jing = scores_batch
            else:
                user_list_jing = np.concatenate((user_list_jing, test_user_batch_array))
                model_pred_jing = np.concatenate((model_pred_jing, scores_batch), axis=0)
        ...
        # jing-dont write
        print(len(user_list_jing))
        print('times_jing: {}'.format(self.times_jing))
        self.times_jing += 1
        # if not self.dataset_is_valid_jing:
        #     save_path = '/home/yinan/jing/model/Pop_predict_baseline_output/traditional_methods/' \
        #                 'tra_method_{}_{}_{}'\
        #         .format(self.dataset_name_jing, self.datetime_jing, self.times_jing)
        #     np.save('{}_user_list'.format(save_path), user_list_jing)
        #     np.save('{}_model_result'.format(save_path), model_pred_jing)
        #     print('model_saved')
        #     print(save_path)


        return results_dict  # , user_list_jing, model_pred_jing
...
class EvaluatorNegativeItemSample(Evaluator):
    ...
    def _run_evaluation_on_selected_users(self, recommender_object, users_to_evaluate, block_size = None):
        ...
        print('EvaluatorNegativeItemSample_jing')
        return results_dict
