...
class GeneralRecommender(AbstractRecommender):
    ...
    def __init__(self, config, dataloader):
        ...
        if not config['end2end'] and config['is_multimodal_model']:
            ...
            # self.v_feat = torch.from_numpy(np.random.normal(loc=0.0, scale=1.0, size=(self.n_items, 4096))).type(torch.FloatTensor).to(
            #          self.device)
            ...
            self.t_feat = torch.from_numpy(np.random.normal(loc=0.0, scale=1.0, size=(self.n_items, 384))).type(torch.FloatTensor).to(
                     self.device)
