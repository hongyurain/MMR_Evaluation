...
class DataHandler:
	...
	def LoadData(self):
		...
		if image: 
			...
			self.image_feats = torch.from_numpy(np.random.normal(loc=0.0, scale=1.0, size=(args.item, 4096))).type(torch.FloatTensor).cuda()
			args.image_feat_dim = 4096
		if text:
			self.text_feats, args.text_feat_dim = self.loadFeatures(self.textfile)
		else:
			self.text_feats = torch.from_numpy(np.random.normal(loc=0.0, scale=1.0, size=(args.item, 384))).type(torch.FloatTensor).cuda()
			args.text_feat_dim = 384
		if args.data == 'tiktok':
