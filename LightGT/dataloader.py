...
def data_load(dataset, has_v=True, has_a=True, has_t=True):
    ...
    if dataset == 'baby':
        ...
        v_feat = torch.tensor(np.load(dir_str + '/v_feats.npy', allow_pickle=True), dtype=torch.float).cuda() if has_v else None
        ...
        t_feat = torch.tensor(np.load(dir_str + '/t_feats.npy', allow_pickle=True), dtype=torch.float).cuda() if has_t else None
    ...
    def get_sparse_graph(self):
        ...
        if self.graph is None:
            try:
                ...
                # print('size',self.user_num + self.item_num)
                adj_mat = adj_mat.tolil()
                train_user = self.edge_index[:, 0]
                # print(self.edge_index[:, 1])
                train_item = self.edge_index[:, 1] - self.user_num
                # print('train_user',train_user, train_item)
                # print('len:', len(train_user))
                # print(train_item)
