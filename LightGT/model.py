...
class Net(nn.Module):
    def __init__(self, user_num, item_num, graph, user_item_dict, v_feat, a_feat, t_feat, eval_dataloader, reg_weight, src_len, batch_size=2048, latent_dim=64, transformer_layers=4, nhead=1, lightgcn_layers=3, score_weight=0.05):
        ...
        # v_feat = torch.from_numpy(np.random.normal(loc=0.0, scale=1.0, size=(self.item_num, 4096))).type(torch.FloatTensor).to(
        #              self.device)
        ...
        self.device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
        self.v_feat = torch.from_numpy(np.random.normal(loc=0.0, scale=1.0, size=(self.item_num, 4096))).type(torch.FloatTensor).to(
                     self.device)
        self.t_feat = torch.from_numpy(np.random.normal(loc=0.0, scale=1.0, size=(self.item_num, 384))).type(torch.FloatTensor).to(
                     self.device)
        self.v_feat = torch.empty((self.item_num, 2048), device=self.device, dtype=torch.float)
        nn.init.kaiming_normal_(self.v_feat, mode='fan_in', nonlinearity='relu')
        
        self.t_feat = torch.empty((self.item_num, 384), device=self.device, dtype=torch.float)
        nn.init.kaiming_normal_(self.t_feat, mode='fan_in', nonlinearity='relu')
    ...
    def forward(self, users, user_item, mask):
        ...
        # print('user_emb',user_emb,user_emb.size())
        # print('item_emb',item_emb,item_emb.size())
        # print('users_mean',users_mean,len(users_mean))
        # print('items_mean',items_mean,len(items_mean))
        # print('user_item',user_item)
        # print('users',users)
        v_src, t_src = [], []
        for i in range(self.transformer_layers):
            # print(items_mean[i],'length:',len(items_mean[i]))
            # print(users_mean[i],'length:',len(users_mean[i]))
            # print(users)
            # print(users.dtype)
            # print(users[1])
            # print(user_item)
            # print(items_mean[i][user_item].detach())
    ...
    def loss(self, users, items, user_item, mask):
        user_emb, item_emb, v, t, v_out, t_out = self.forward(users[:, 0], user_item, mask.cuda())
        ...
        if v is not None and t is not None:
            score2_1 = torch.sum(v_out * v[pos_items], dim=1).view(-1, 1) + torch.sum(t_out * t[pos_items], dim=1).view(-1, 1)
            score2_2 = torch.sum(v_out * v[neg_items], dim=1).view(-1, 1) + torch.sum(t_out * t[neg_items], dim=1).view(-1, 1)
        elif v is not None:
            score2_1 = torch.sum(v_out * v[pos_items], dim=1).view(-1, 1)
            score2_2 = torch.sum(v_out * v[neg_items], dim=1).view(-1, 1)
        else:
            score2_1 = torch.sum(t_out * t[pos_items], dim=1).view(-1, 1)
            score2_2 = torch.sum(t_out * t[neg_items], dim=1).view(-1, 1)
