...
class Model(object):
    ...
    def __init__(self, data_config, text_feat, pop_feat, d2):
    ...
    def create_bpr_loss_int(self):
        ...
        mf_loss_int_m = tf.reduce_mean(tf.nn.softplus(-(pos_scores_int_t - neg_scores_int_t)))
        #mf_loss_int_m = tf.reduce_mean(tf.nn.softplus(-(pos_scores_int_v - neg_scores_int_v)))
        ...
        emb_loss_int = self.decay * regularizer_mf_int_t / self.batch_size
        ...
        self.user_int_embed_t = tf.nn.l2_normalize(self.u_g_int_embeddings_t_pre, axis=1)
        self.item_int_embed_t = tf.nn.l2_normalize(self.pos_i_g_int_embeddings_t_pre, axis=1)
    ...
    def create_bpr_loss_con(self):
        ...
        mf_loss_m_con = tf.reduce_mean(tf.nn.softplus(-(pos_scores_con_t - neg_scores_con_t)))

        emb_loss_con = self.decay * regularizer_mf_con_t / self.batch_size

        self.user_con_embed_t = tf.nn.l2_normalize(self.u_g_con_embeddings_t_pre, axis=1)
        self.item_con_embed_t = tf.nn.l2_normalize(self.pos_i_g_con_embeddings_t_pre, axis=1)
    ...
    def _dis_loss(self):
        ...
        discrepancy_loss = self.mean_cos_dis(self.u_g_int_embeddings_t, self.u_g_con_embeddings_t) + \
                           self.mean_cos_dis(self.pos_i_g_int_embeddings_t, self.pos_i_g_con_embeddings_t)
...
if __name__ == '__main__':
    ...
    model = Model(data_config=config,
                  text_feat=data_generator.textFeatMatrix,
                  pop_feat=data_generator.popFeaMatrix, d2=d2)
