...
def test_one_user(x):
    ...
    return {'recall': np.array(recall), 'ndcg': np.array(ndcg), 'hit_ratio': np.array(hit_ratio), 'k': (u, K_max_item_score[:100])}
...
if __name__ == '__main__':
    ...
    random_image = tf.random_normal([ITEM_NUM, d1], mean=0.0, stddev=1.0, dtype=tf.float32)
    random_text = tf.random_normal([ITEM_NUM, d2], mean=0.0, stddev=1.0, dtype=tf.float32)
    
    if random:
        model = Model(data_config=config,
                      img_feat=random_image,
                      text_feat=random_text,
                      pop_feat=data_generator.popFeaMatrix, d1=d1, d2= d2)
    else:
        model = Model(data_config=config,
    ...
    for epoch in range(500):
        ...
        if score > best_score:
            ...
            print('best result until now, epoch {}: recall@1,5,10,20,50,100={:.4f},{:.4f},{:.4f},{:.4f},{:.4f},{:.4f},hr@1,5,10,20,50,100={:.4f},{:.4f},{:.4f},{:.4f},{:.4f},{:.4f},ndcg@1,5,10,20,50,100={:.4f},{:.4f},{:.4f}{:.4f},{:.4f},{:.4f}'.format(
                best_epoch, recall[0], recall[4], recall[9], recall[19],recall[49], recall[99], hr[0], hr[4], hr[9], hr[19], hr[49], hr[99], ndcg[0], ndcg[4], ndcg[9], ndcg[19], ndcg[49], ndcg[99]))
            
            # with open('./topk/epoch{}-saving_test.pkl'.format(best_epoch),'w') as file:
            #     pickle.dump(k, file)
            # for item in k:
            #     file.writelines(item)
            # file.close()

            csv_file = "./topk/epoch{}-topk_results.csv".format(best_epoch)
            with open(csv_file, mode='w', newline='') as f:
                writer = csv.writer(f)

                # header
                top_k_len = len(k[0][1])
                header = ['id'] + [f'top_{i+1}' for i in range(top_k_len)]
                writer.writerow(header)

                # each row
                for uid, topk_items in k:
                    row = [uid] + topk_items
                    writer.writerow(row)
        ...
        perf_str = 'Epoch {} [{:1f}s + {:1f}s]: recall@1=[{:5f}], recall@5=[{:5f}],recall@10=[{:5f}],recall@20=[{:5f}],recall@50=[{:5f}],recall@100=[{:5f}],hit@1=[{:5f}], hit@5=[{:5f}],hit@10=[{:5f}],hit@20=[{:5f}],hit@50=[{:5f}],hit@100=[{:5f}],ndcg@1=[{:5f}],ndcg@5=[{:5f}],ndcg@10=[{:5f}],ndcg@20=[{:5f}],ndcg@50=[{:5f}],ndcg@100=[{:5f}]'.format(epoch, t2 - t1, t3 - t2,
                    recall[0], recall[4], recall[9], recall[19],recall[49], recall[99], hr[0], hr[4], hr[9], hr[19], hr[49], hr[99], ndcg[0], ndcg[4], ndcg[9], ndcg[19], ndcg[49], ndcg[99])
        print(perf_str)
        file.write(perf_str + '\n')
        ...
        if early_stopping == 10:
            break
    print('###########################################################################################################################')
    print('[{}], epoch {} best result: recall@1,5,10,20,50,100={},{},{},{},{},{},hr@1,5,10,20,50,100={},{},{},{},{},{},ndcg@1,5,10,20,50,100={},{},{},{},{},{}'.format(dataset,best_epoch,
        best_result['recall'][0], best_result['recall'][4], best_result['recall'][9], best_result['recall'][19], best_result['recall'][49], best_result['recall'][99], best_result['hr'][0], best_result['hr'][4], best_result['hr'][9], best_result['hr'][19], best_result['hr'][49], best_result['hr'][99], best_result['ndcg'][0], best_result['ndcg'][4],
        best_result['ndcg'][9], best_result['ndcg'][19], best_result['ndcg'][49], best_result['ndcg'][99]))
    file.write('[{}], epoch {} best result: recall@1,5,10,20,50,100={},{},{},{},{},{},hr@1,5,10,20,50,100={},{},{},{},{},{},ndcg@1,5,10,20,50,100={},{},{},{},{},{}'.format(dataset,best_epoch,
        best_result['recall'][0], best_result['recall'][4], best_result['recall'][9], best_result['recall'][19], best_result['recall'][49], best_result['recall'][99], best_result['hr'][0], best_result['hr'][4], best_result['hr'][9], best_result['hr'][19], best_result['hr'][49], best_result['hr'][99], best_result['ndcg'][0], best_result['ndcg'][4],
        best_result['ndcg'][9], best_result['ndcg'][19], best_result['ndcg'][49], best_result['ndcg'][99]) + '\n')
    file.close()
