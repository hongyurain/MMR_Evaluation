...
class Model(object):
    ...
    def __init__(self, data_config, text_feat, d2):
        ...
        self.pos_inputs_embeddings = tf.concat([self.pos_i_g_embeddings, self.pos_i_g_embeddings_t], axis=1)
        self.neg_inputs_embeddings = tf.concat([self.neg_i_g_embeddings, self.neg_i_g_embeddings_t], axis=1)
        self.int_inputs_embeddings = tf.concat([self.int_i_g_embeddings, self.int_i_g_embeddings_t], axis=1)
        ...
        self.multiply = tf.reduce_sum(self.u_g_embeddings * self.pos_i_g_embeddings, 1) + \
                        lambda_m*tf.reduce_sum(self.u_g_embeddings * self.pos_i_g_embeddings_t, 1) + \
    ...
    def counterfactual_learning_layer_1(self, query, key_value, query_int, key_value_int, activation=None, name=None):
        ...
        with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
            ...
            counterfactual_learning = tf.layers.dense(attention, units=2 * self.emb_dim, activation=activation,
    ...
    def create_bpr_loss(self):
        ...
        emb_loss = self.decay * (regularizer_mf + regularizer_mf_t + regularizer_mf_m) / self.batch_size

        loss = mf_loss + emb_loss
        self.user_embed = tf.nn.l2_normalize(self.u_g_embeddings_pre, axis=1)
        self.item_embed, self.item_embed_t = tf.nn.l2_normalize(self.pos_i_g_embeddings_pre, axis=1), \
    ...
    def _evaluate(self, sess):
        ...
        for user, item, pos in data_generator.test_data():
            feed = {
                self.users: user,
                self.pos_items: item,
                self.int_items: item
        ...
        # return hr_5, hr_10, hr_20, ndcg_5, ndcg_10, ndcg_20
            ...
            # Track total hits and total relevant items
            hit_num_5 += hit_5
            hit_num_10 += hit_10
            hit_num_20 += hit_20
            # total_relevant += len(pos)

            # Calculate recall
            recall_5 = hit_5 / len(pos) if len(pos) > 0 else 0
            recall_10 = hit_10 / len(pos) if len(pos) > 0 else 0
            recall_20 = hit_20 / len(pos) if len(pos) > 0 else 0
        ...
        #Average recall over all tests
        avg_recall_5 = total_recall_5 / test_num if test_num > 0 else 0
        avg_recall_10 = total_recall_10 / test_num if test_num > 0 else 0
        avg_recall_20 = total_recall_20 / test_num if test_num > 0 else 0

        # Average NDCG over all tests
        avg_ndcg_5 = total_ndcg_5 / test_num if test_num > 0 else 0
        avg_ndcg_10 = total_ndcg_10 / test_num if test_num > 0 else 0
        avg_ndcg_20 = total_ndcg_20 / test_num if test_num > 0 else 0

        return hr_5, hr_10, hr_20, avg_ndcg_5, avg_ndcg_10, avg_ndcg_20, avg_recall_5, avg_recall_10, avg_recall_20
...
if __name__ == '__main__':
    ...
    model = Model(data_config=config, text_feat=data_generator.textFeatMatrix, d2=t_emb)
    ...
    for epoch in range(epochs):
        ...
        *********************************************************
        ...
        """
        t2 = time.time()
        hr_5, hr_10, hr_20, ndcg_5, ndcg_10, ndcg_20, recall_5, recall_10, recall_20 = model._evaluate(sess)

        t3 = time.time()

        perf_str = 'Epoch %d [%.1fs + %.1fs]: train==[%.5f=%.5f + %.5f],hit@5=[%.5f],hit@10=[%.5f],hit@20=[%.5f],ndcg@5=[%.5f],ndcg@10=[%.5f],ndcg@20=[%.5f], recall@5=[%.5f],recall@10=[%.5f],recall@20=[%.5f]' % \
                   (epoch, t2 - t1, t3 - t2, loss, mf_loss, emb_loss, hr_5, hr_10, hr_20, ndcg_5, ndcg_10, ndcg_20, recall_5, recall_10, recall_20)
        print(perf_str)
        file.write(perf_str + '\n')
