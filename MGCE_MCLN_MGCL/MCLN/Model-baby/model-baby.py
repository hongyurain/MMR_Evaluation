...
if __name__ == '__main__':
    ...
    if random:
        random_image = tf.random_normal([ITEM_NUM, 4096], mean=0.0, stddev=1.0, dtype=tf.float32)
        random_text = tf.random_normal([ITEM_NUM, 384], mean=0.0, stddev=1.0, dtype=tf.float32)
        model = Model(data_config=config, img_feat=random_image, text_feat=random_text, d1=v_emb, d2=t_emb)
    else:
        model = Model(data_config=config, img_feat=data_generator.imageFeatMatrix, text_feat=data_generator.textFeatMatrix, d1=v_emb, d2=t_emb)
