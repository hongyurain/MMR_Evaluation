...
class Data(object):
    def __init__(self, path, batch_size):
        ...
        if img_enable:
            ...
            with open(img_feat_file, 'r') as file:
                ...
                self.imageFeatMatrix = [[0.] * self.v_emb] * self.n_items
        ...
        if title_enable:
            ...
            with open(text_feat_file, 'r') as file:
                ...
                self.textFeatMatrix = [[0.] * self.t_emb] * self.n_items
    ...
    def test_data(self):
        for u, i in self.test_set.items():
            ...
            # Initialize user and item batches
            test_items = []
            negative_items = []
            for h in range(self.n_items):
                if h in self.train_items[u] or h in i:
                    continue
                negative_items.append(h)
            test_items.extend(negative_items)
            for j in i:
            ...
            user_batch = [u] * len(test_items)  # Set all entries to the current user
            item_batch = test_items # Include all items as test items

            # Yield the batch for this user
            yield user_batch, item_batch, i
