...
class Trainer(object):
    def __init__(self, data_config):
        ...
        self.ui_graph = self.ui_graph_raw = pickle.load(open(args.data_path + args.dataset + '/train_mat.pkl','rb'))
        ...
        #     self.text_feats = torch.from_numpy(np.random.normal(loc=0.0, scale=1.0, size=(self.n_items, 384))).type(torch.FloatTensor).cuda()
        # else:
        ...
        # text only
        # self.text_feats = np.load(args.data_path + '{}/text_feat.npy'.format(args.dataset))
        # self.image_feats = torch.from_numpy(np.random.normal(loc=0.0, scale=1.0, size=(self.n_items, 4096))).type(torch.FloatTensor).cuda()
    ...
    def train(self):
        ...
        print('best recall@20:', best_recall,':result:', str(test_ret))
